...
export const generateWebhookHmac = (payload, secret) => {
    return crypto
        .createHmac('sha256', secret)
        .update(payload, 'utf8')
        .digest('hex');
};
export const main = async (PORT = 3000, secret = String(process.env.TBA_SECRET), redisName = String(process.env.REDIS_NAME)) => {
    const app = express();
    ...
    const logEvent = async (event, data) => {
        ...
        await fs.appendFile('../logs/events.log', `${new Date().toISOString()} - ${event}: ${JSON.stringify(data)}\n`);
    };
    const parseTBAEvent = (data) => {
    ...
    app.post('/', bodyParser.text({ type: 'application/json' }), async (req, res) => {
        try {
